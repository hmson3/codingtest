    ...
    while (!green.empty() && !red.empty()) {
        ...
        while (!red.empty() && dis_red[red.front().x][red.front().y] == time - 1) {
            ...
            for (int dir = 0; dir < 4; dir++) {
                ...
                if(dis_red[cur.x][cur.y] + 1 == dis_green[nx][ny]) {
                    dis_red[nx][ny] = dis_red[cur.x][cur.y] + 1;
                    tmp[nx][ny] = 3;
                    t++;
                }
                if (dis_green[nx][ny] >= 0) continue; 
                dis_red[nx][ny] = dis_red[cur.x][cur.y] + 1;
                red.push({nx, ny});
            }
        }
...
int main() {
    ...
    cand = baeyang.size();
    fill(baeyangtack + cand - r - g, baeyangtack + cand - r, 1);
    fill(baeyangtack + cand - r, baeyangtack + cand, 2);
    do {
        for (int i = 0; i < n; i++) {
        ...
        ans = max(ans, solve());
    } while (next_permutation(baeyangtack, baeyangtack + cand));
